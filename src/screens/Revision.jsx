export default function Revision({ chapter, exams, onSelectExam, onBack }) {
  return (
    <div className="bg-white flex flex-col" style={{ height: '100dvh', overflow: 'hidden' }}>
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden px-5 pb-10 flex flex-col gap-3">
        {exams.map((exam, i) => (
          <button
            key={exam.id}
            onClick={() => onSelectExam(exam)}
            className="w-full text-left bg-white border-2 border-gray-100 rounded-2xl px-5 py-4 transition-all active:translate-y-1 overflow-hidden"
            style={{ boxShadow: '0 4px 0 #e5e7eb', maxWidth: '100%' }}
